...
const Procurement: React.FC = () => {
  ...
  const handleAdvanceStatus = async (orderId: string, action: 'submit' | 'approve' | 'order') => {
    setActionLoading(true);
    setActionMsg('');
    try {
      const res = await api.put(`/procurement/${orderId}/${action}`);
      const updated = res.data.data;
      setOrders(prev => prev.map(o => o.id === orderId ? { ...o, status: updated.status } : o));
      setSelectedOrder((prev: any) => prev ? { ...prev, status: updated.status } : null);
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-muted/20 border-b border-border/50">
              ...
              <th className="px-8 py-5 text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em]">Current Phase</th>
        ...
        {selectedOrder && (
          <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4 bg-background/60 backdrop-blur-xl" onClick={() => setSelectedOrder(null)}>
            <motion.div
              ...
              className="bg-card/80 border border-border/50 w-full max-w-xl rounded-[2.5rem] overflow-hidden shadow-[0_32px_64px_-12px_rgba(0,0,0,0.3)] glass"
            >
              {/* Header */}
              <div className="px-8 py-7 border-b border-border/50 flex items-center justify-between bg-muted/10">
                <div>
                  <p className="text-[10px] uppercase tracking-[0.3em] text-muted-foreground font-black">LOGISTICS INTELLIGENCE</p>
                  <h3 className="text-2xl font-black text-foreground font-mono tracking-tighter mt-1">PO-{selectedOrder.id.split('-')[0].toUpperCase()}</h3>
                </div>
                <button onClick={() => setSelectedOrder(null)} className="w-10 h-10 flex items-center justify-center bg-muted/50 hover:bg-rose-500/10 hover:text-rose-500 rounded-full text-muted-foreground transition-all">
              ...
              <div className="p-8 space-y-8 max-h-[70vh] overflow-y-auto custom-scrollbar">
                ...
                <div className="grid grid-cols-2 gap-6">
                  <div className="bg-background/40 border border-border/50 rounded-3xl p-5 shadow-sm">
                    <p className="text-[10px] uppercase tracking-[0.2em] text-muted-foreground font-black mb-2">SUPPLIER NODE</p>
                    <p className="font-black text-foreground text-lg leading-tight">{selectedOrder.vendor?.name}</p>
                    <p className="text-[10px] text-muted-foreground/60 uppercase tracking-widest mt-1">Verified Gateway</p>
                  </div>
                  <div className="bg-primary/5 border border-primary/20 rounded-3xl p-5 shadow-inner">
                    <p className="text-[10px] uppercase tracking-[0.2em] text-primary font-black mb-2">COMMIT VALUE</p>
                    <p className="font-black text-primary text-2xl tracking-tighter">₹{Number(selectedOrder.totalAmount).toLocaleString('en-IN')}</p>
                  </div>
                </div>

                {/* Status Journey */}
                <div className="bg-background/40 border border-border/50 rounded-3xl p-6">
                  <p className="text-[10px] uppercase tracking-[0.2em] text-muted-foreground font-black mb-5">LIFECYCLE TRAJECTORY</p>
                  <div className="flex items-center gap-1.5 ">
                    {['DRAFT', 'SUBMITTED', 'APPROVED', 'ORDERED', 'COMPLETED'].map((s, idx) => {
                      const sc = statusConfig[s] || statusConfig.COMPLETED;
                      const active = selectedOrder.status === s || (s === 'ORDERED' && selectedOrder.status === 'PARTIALLY_RECEIVED');
                      const done = ['DRAFT', 'SUBMITTED', 'APPROVED', 'ORDERED', 'PARTIALLY_RECEIVED', 'COMPLETED'].indexOf(selectedOrder.status) >= ['DRAFT', 'SUBMITTED', 'APPROVED', 'ORDERED', 'COMPLETED'].indexOf(s) && selectedOrder.status !== s;
                      
                      return (
                        <React.Fragment key={s}>
                          <div className={`flex-1 text-center py-2 px-1 rounded-xl text-[8px] font-black uppercase tracking-widest border transition-all ${
                            active ? `${sc.bg} ${sc.color} ${sc.border} shadow-sm scale-105` :
                            done ? 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20 opacity-60' :
                            'bg-muted/30 text-muted-foreground/40 border-border/30'
                          }`}>
                            {done ? '✓ ' : ''}{s === 'COMPLETED' ? 'DONE' : s}
                          </div>
                          {idx < 4 && <div className="text-border/40 text-[10px]">/</div>}
                        </React.Fragment>
                      );
                    })}
                  </div>
                ...
                {actionMsg && (
                  <motion.div 
                    ...
                    className={`p-4 rounded-2xl text-xs font-black uppercase tracking-widest text-center border shadow-inner ${
                ...
                <div className="space-y-3 pt-2">
                  {selectedOrder.status === 'DRAFT' && (
                    <button
                      ...
                      className="w-full h-14 flex items-center justify-center gap-3 bg-amber-500 hover:bg-amber-400 text-white font-black uppercase tracking-[0.2em] text-xs rounded-2xl transition-all shadow-lg shadow-amber-500/20 active:scale-95 disabled:opacity-30"
                    >
                      <Send size={18} strokeWidth={3} /> {actionLoading ? 'PROCESSING...' : 'INITIATE APPROVAL'}
                    </button>
                  )}
                  {selectedOrder.status === 'SUBMITTED' && (
                    <button
                      disabled={actionLoading || !hasPermission(PERMISSIONS.PROCUREMENT_APPROVE)}
                      onClick={() => handleAdvanceStatus(selectedOrder.id, 'approve')}
                      className="w-full h-14 flex items-center justify-center gap-3 bg-primary hover:bg-primary/90 text-white font-black uppercase tracking-[0.2em] text-xs rounded-2xl transition-all shadow-lg shadow-primary/20 active:scale-95 disabled:opacity-30"
                    >
                      <ThumbsUp size={18} strokeWidth={3} /> {actionLoading ? 'VERIFYING...' : 'AUTHORIZE PROCUREMENT'}
                    </button>
                  )}
                  {selectedOrder.status === 'APPROVED' && (
                    <button
                      ...
                      className="w-full h-14 flex items-center justify-center gap-3 bg-violet-600 hover:bg-violet-500 text-white font-black uppercase tracking-[0.2em] text-xs rounded-2xl transition-all shadow-lg shadow-violet-500/20 active:scale-95 disabled:opacity-30"
                    >
                      <Package size={18} strokeWidth={3} /> {actionLoading ? 'SYNCING...' : 'MARK AS DISPATCHED'}
                    </button>
                  )}
                  {(selectedOrder.status === 'ORDERED' || selectedOrder.status === 'PARTIALLY_RECEIVED') && (
                    <button
                      ...
                      className="w-full h-14 flex items-center justify-center gap-3 bg-emerald-600 hover:bg-emerald-500 text-white font-black uppercase tracking-[0.2em] text-xs rounded-2xl transition-all shadow-lg shadow-emerald-500/20 active:scale-95 disabled:opacity-30"
                    >
                      <PackageCheck size={18} strokeWidth={3} /> LOG INBOUND SHIPMENT
                    </button>
                  )}
                  {selectedOrder.status === 'COMPLETED' && (
                    <div className="text-center py-5 text-emerald-500 font-black uppercase tracking-widest text-[10px] flex items-center justify-center gap-3 border-2 border-dashed border-emerald-500/20 bg-emerald-500/5 rounded-2xl">
            ...
            >
                  ...
                  <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-500">
                    <div className="flex items-center justify-between mb-8 bg-background/40 p-6 rounded-3xl border border-border/50 shadow-sm">
                      ...
                      <div className="relative w-80 group">
                        ...
                        <select 
                          onChange={(e) => { addItemToOrder(e.target.value); e.target.value = ''; }}
                          className="w-full bg-card border-2 border-border/50 rounded-2xl py-3 pl-12 pr-4 text-xs font-black uppercase tracking-widest text-foreground focus:outline-none focus:border-primary/50 focus:ring-4 focus:ring-primary/5 transition-all appearance-none cursor-pointer"
                          defaultValue=""
                        >
                          <option value="" disabled>Search Global Repository...</option>
                          {products.map((p: any) => (
                ...
                {step === 3 && (
                  <div className="space-y-8 animate-in zoom-in-95 duration-500">
                    <div className="text-center space-y-2 mb-10">
                      ...
                      <p className="text-muted-foreground font-medium">Verify the procurement parameters before broadcast.</p>
                    </div>
                    ...
                    <div className="bg-amber-500/5 border border-amber-500/20 rounded-[1.5rem] p-6 flex items-start space-x-4">
                      ...
                      <p className="text-xs text-amber-600 dark:text-amber-400 leading-relaxed font-bold uppercase tracking-wide">
                        Broadcast Warning: This request will initiate in <strong className="underline">DRAFT</strong> mode. Final stock synchronization occurs post-receipt verification in the logistics layer.
                      </p>
                    </div>
                  </div>
                )}
              ...
              <div className="px-10 py-7 bg-muted/20 border-t border-border/40 flex justify-between items-center">
                ...
                {step < 3 ? (
                  ...
                  >
                    <Send size={18} className="mr-3" strokeWidth={3} /> Broadcast Lifecycle
                  </button>
                )}
              </div>
            </motion.div>
